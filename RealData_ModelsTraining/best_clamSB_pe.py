...
#0.8016 K:6, Drop:0, cC:0.7, LR:1e-5, WD:1e-7
cK = 6
cDrop = 0 
cC = 0.7
cLR = 1e-5
...
for epoch in range(100):
    ...
    for batch_idx, (curridx, data, target) in enumerate(train_loader):
        ...
        bag_prediction, inst_dict = model(data, target.to(torch.int64), instance_eval = True)
        loss_bag = cC*criterion(bag_prediction, target.long())
        ...
        losses.append(loss_bag.detach().detach().cpu().item())
        del data
        del bag_prediction
        del loss_bag
    ...
    torch.save(model.state_dict(), '/data2/meerak/models/best_clamSB_pe_mimiccxr_densenet_epoch%d'%(epoch))
    ...
    with open('best_clamSB_pe.txt', 'w') as f:
        count = 0
        f.write('tl, ta, vl, va, ta\n')
        f.write('K:%d, Drop:%d, cC:%0.1f, LR:1e%d, WD:1e%d\n'%(cK, cDrop, cC, np.log10(cLR), np.log10(cWD)))
