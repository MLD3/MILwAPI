...
for num_bags in [100, 1000, 10000]:
    ...
    for perm_dist in range(0, 11):
        train_shuffle_idxs = joblib.load('../MNIST/train_idxs_numbags_%d_permdist_%d.joblib'%(num_bags, perm_dist))
        val_shuffle_idxs = joblib.load('../MNIST/val_idxs_numbags_%d_permdist_%d.joblib'%(num_bags, perm_dist))
        test_shuffle_idxs = joblib.load('../MNIST/test_idxs_permdist_%d.joblib'%perm_dist)
        ...
        for seed in range(10):
            ...
            for epoch in range(500):
                ...
                for curridx, x_i, y_i, actual_y_i in train_loader:
                    ...
                    x_i = x_i[train_shuffle_idxs[curridx]].to(device)
                    actual_y_i = actual_y_i.squeeze(0)[train_shuffle_idxs[curridx]]

                    # positive pair, with encoding
                    bag_prediction, loss_bag = model(x_i, y_i, criterion)
                ...
                for curridx, x_i, y_i, actual_y_i in val_loader:
                    ...
                    x_i = x_i[val_shuffle_idxs[curridx]].to(device)
                ...
                for curridx, x_i, y_i, actual_y_i in test_loader:
                    ...
                    x_i = x_i[test_shuffle_idxs[curridx]].to(device)
                ...
                with open('../txtfiles/seed%d_numbags%d_MNIST_permdist%d_dtfd_pe_loss.txt'%(seed, num_bags, perm_dist), 'w') as f:
                    count = -1
