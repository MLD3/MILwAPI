...
for num_bags in [100, 1000, 10000]:
    X_train = joblib.load('/data2/meerak/MNIST/X_train_%d_%d_%d'%((int(sparsity*100), bag_size, num_bags)))
    y_train = joblib.load('/data2/meerak/MNIST/y_train_%d_%d_%d'%((int(sparsity*100), bag_size, num_bags)))
    actual_y_train = joblib.load('/data2/meerak/MNIST/actual_y_train_%d_%d_%d'%((int(sparsity*100), bag_size, num_bags)))

    X_val = joblib.load('/data2/meerak/MNIST/X_val_%d_%d_%d'%((int(sparsity*100), bag_size, num_bags)))
    y_val = joblib.load('/data2/meerak/MNIST/y_val_%d_%d_%d'%((int(sparsity*100), bag_size, num_bags)))
    actual_y_val = joblib.load('/data2/meerak/MNIST/actual_y_val_%d_%d_%d'%((int(sparsity*100), bag_size, num_bags)))
    ...
    for perm_dist in range(0, 11):
        train_shuffle_idxs = joblib.load('/data2/meerak/MNIST/train_idxs_numbags_%d_permdist_%d.joblib'%(num_bags, perm_dist))
        val_shuffle_idxs = joblib.load('/data2/meerak/MNIST/val_idxs_numbags_%d_permdist_%d.joblib'%(num_bags, perm_dist))
        test_shuffle_idxs = joblib.load('/data2/meerak/MNIST/test_idxs_permdist_%d.joblib'%perm_dist)
        ...
        for seed in range(10):
            ...
            model = SGLModel(ftEx, cM, cE, cL, cD).to(device)
            ...
            for epoch in range(500):
                ...
                for curridx, x_i, y_i, actual_y_i in train_loader:
                    ...
                    loss_bag, loss_inst = sgl(inst_pred, y_i.long(), delta = 0.2, pool_before_act = 1)

                    final_loss = loss_bag + 0.5*loss_inst
                ...
                with open('/data2/meerak/txtfiles/seed%d_numbags%d_MNIST_permdist%d_sglmodel_loss.txt'%(seed, num_bags, perm_dist), 'w') as f:
                    count = -1
